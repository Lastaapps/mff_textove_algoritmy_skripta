...
3. A stable sort (like radix sort) on these pairs `(T[i], Rank[i+1])` gives the sorted order of the suffixes at `i mod 3 = 0`.
...
The suffixes at positions `i mod 3 = 0` are sorted based on pairs of values: `(T[i], Rank[i+1])`.
- `$T[i]$` is the first character of the suffix.
- `$Rank[i+1]$` is the lexicographical rank of the suffix starting at `i+1`. Since `(i+1) mod 3 = 1`, this rank is known from the first step of the algorithm (the sorting of suffixes at positions not divisible by 3).
