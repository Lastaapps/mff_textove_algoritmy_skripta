...
#info_box(
  ...
  - A special terminator character, usually `\$`, is appended to the string (`sigma\$`) to ensure that every suffix ends at a unique leaf node in the trie. This prevents one suffix from being a prefix of another suffix.
]
...
#info_box(
  title: "Problem: Suffix Trie Space Requirement",
)[
...
#info_box(
  title: "Patricia Trie (Compressed Trie)",
)[
...
A *Suffix Tree* is essentially a *Compressed Suffix Trie* built for a word $sigma\$[0...n-1]$. It is a Patricia Trie built from all suffixes of a given string. This compression significantly reduces the space complexity while retaining the fast search capabilities.

#info_box(
  title: "Suffix Tree Definition Refined",
)[
  - It is a compact trie constructed from the set of all suffixes of a string $sigma$.
  - A common practice is to append a unique terminator character `\$` to the string (e.g., $sigma\$$) to ensure that all suffixes end at leaf nodes and no suffix is a prefix of another.
...
#info_box(
  title: "Example: Suffix Tree for 'abaabaab'",
)[
...
#info_box(
  title: "Explicit vs. Implicit Representation",
)[
  - *In a Suffix Trie*:
...
#info_box(
  title: "Linear Space Bounds for Suffix Trees",
)[
...
#info_box(
  title: "Edge Label Representation and Space",
)[
...
#info_box(
  title: "Substring Search in a Suffix Tree",
)[
...
#info_box(
  title: "Representing Nodes and Edges",
)[
...
#info_box(
  title: "Node Representation Strategies",
)[
  - *1. Linked List*:
    - *Branching Time*: $O(alpha)$ (proportional to the number of children $alpha$ of the current node).
    - *Total Search Time*: $O(m * alpha)$, where $m$ is pattern length.
  - *2. Ordered List*:
  ...
  - *3. Binary Search Tree (BST)*:
    ...
    - *Total Search Time*: $O(m * log alpha)$.
...
#info_box(
  title: "Pattern Matching Approaches",
)[
  ...
  - *General Steps*:
    ...
    2. For the current node and the current character $iota[i]$ of the pattern, select an outgoing edge whose label $gamma$ starts with $iota[i]$.
  ...
  - *Slowscan Approach*:
    ...
    - *Time Complexity*: $O(B * m)$, where $B$ is the time for branching (selecting an edge, e.g., $O(1)$, $O(log alpha)$, $O(alpha)$) and $m = |iota|$ is the length of the pattern. This can be slow if $m$ is large and $B$ is not $O(1)$.
  ...
  - *Fastscan Approach (Optimized)*:
    ...
    - *Time Complexity*: $O(B * k)$, where $B$ is the branching time, and $k$ is the number of visited nodes. Since $k <= |iota|$, this is more efficient as character comparisons along edges are avoided.
]
...
#info_box(
  title: "Key In-Memory Algorithms",
)[
  - *Weiner (1973)*:
    - *Time Complexity*: $O(n log alpha)$ over an ordered alphabet.
  - *McCreight (1976)*:
    - An improvement on Weiner's algorithm.
    - *Time Complexity*: $O(n log alpha)$ over an ordered alphabet.
    - Constructs the tree by incrementally inserting suffixes $sigma[i:]\$$ for $i = 0..n$.
  - *Ukkonen (1992)*:
    - *Time Complexity*: $O(n log alpha)$ over an ordered alphabet.
    ...
    - Constructs the tree by incrementally inserting prefixes $sigma[:i]$ for $i = 1..n$ and finally the `\$` terminator.
  - *Farach (1997)*:
