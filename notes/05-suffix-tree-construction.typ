...
Their power becomes clear when we perform a sequence of extensions. Suppose we just inserted a suffix $alpha w$ by splitting an edge and creating a new internal node $u$. To insert the next suffix, $w$, we would normally have to go back to the root and traverse the path for $w$ from scratch. However, if $u$ has a suffix link to $v$ (which represents $w$), we can simply jump to $v$ in a single step. This saves a significant amount of re-traversal.
...
=== Solution 4
A suffix link from a node $u$ representing $alpha w$ to a node $v$ representing $w$ allows for quick traversal between suffixes. This is a key optimization for achieving linear-time complexity.
